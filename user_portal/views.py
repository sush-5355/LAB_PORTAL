...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        if user_id:
            user = User.objects.get(id=user_id)  # Query the user from the database using the ID
...
def slot_booking(request):
    if request.method == 'POST':
        ...
        if selected_date:
            ...
            if not existing_slots:
                ...
                for i in range(6):
                    ...
                    start_time = (datetime.combine(datetime.today(), start_time) + timedelta(hours=4)).time()
        ...
        return render(request, 'slot_booking.html', {'slots': slots, 'selected_date': selected_date})
...
def my_slots(request):
    ...
    booked_slots = Slot.objects.filter(user=user, status=True)  # Assuming `status=True` means booked
